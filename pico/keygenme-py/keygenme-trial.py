...
def check_key(key, username_trial):
    ...
    # 1. Determined that the length of the key is 32 characters.
    print("Length of key_full_template_trial: {}".format(len(key_full_template_trial)))
    print("Length of key:                     {}".format(len(key)))
    if len(key) != len(key_full_template_trial):
        return False
    else:
        # Check static base key part --v

        # 2. Confirm that the inputted key passed the length check.
        ...
        # 3. Printing out the expected characters.
